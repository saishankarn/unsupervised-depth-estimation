...
# Incorporated loss functions from monodepth paper, no  multiple scales approach involved in this code. good improvement when compared to older versions 
...
class Correspondence():
	...
	def __init__(self, iS, iT):
		...
		self.model_input = tf.concat([self.input_S, self.input_T], 3)

		with tf.variable_scope('conv_layers_1_to_16') as scope:

			self.conv1    = tf.nn.leaky_relu(tf.layers.conv2d(self.model_input, 8,   (9, 9), (1, 1), padding = 'same', name = 'conv_layer_1' ), alpha = 0.1)
			self.conv2    = tf.nn.leaky_relu(tf.layers.conv2d(self.conv1,       8,   (9, 9), (2, 2), padding = 'same', name = 'conv_layer_2' ), alpha = 0.1) #(N_ROWS/2, N_COLS/2)
			self.conv3    = tf.nn.leaky_relu(tf.layers.conv2d(self.conv2,       16,  (9, 9), (1, 1), padding = 'same', name = 'conv_layer_3' ), alpha = 0.1) 
			self.conv4    = tf.nn.leaky_relu(tf.layers.conv2d(self.conv3,       16,  (9, 9), (2, 2), padding = 'same', name = 'conv_layer_4' ), alpha = 0.1) #(N_ROWS/4, N_COLS/4)

			self.conv5    = tf.nn.leaky_relu(tf.layers.conv2d(self.conv4,       32,  (7, 7), (1, 1), padding = 'same', name = 'conv_layer_5' ), alpha = 0.1)
			self.conv6    = tf.nn.leaky_relu(tf.layers.conv2d(self.conv5,       32,  (7, 7), (2, 2), padding = 'same', name = 'conv_layer_6' ), alpha = 0.1) #(N_ROWS/8, N_COLS/8)
			self.conv7    = tf.nn.leaky_relu(tf.layers.conv2d(self.conv6,       64,  (7, 7), (1, 1), padding = 'same', name = 'conv_layer_7' ), alpha = 0.1)
			self.conv8    = tf.nn.leaky_relu(tf.layers.conv2d(self.conv7,       64,  (7, 7), (2, 2), padding = 'same', name = 'conv_layer_8' ), alpha = 0.1) #(N_ROWS/16, N_COLS/16)

			self.conv9    = tf.nn.leaky_relu(tf.layers.conv2d(self.conv8,       128, (5, 5), (1, 1), padding = 'same', name = 'conv_layer_9' ), alpha = 0.1)
			self.conv10   = tf.nn.leaky_relu(tf.layers.conv2d(self.conv9,		128, (5, 5), (2, 2), padding = 'same', name = 'conv_layer_10'), alpha = 0.1) #(N_ROWS/32, N_COLS/32)
			self.conv11   = tf.nn.leaky_relu(tf.layers.conv2d(self.conv10,      256, (5, 5), (1, 1), padding = 'same', name = 'conv_layer_11'), alpha = 0.1)
			self.conv12   = tf.nn.leaky_relu(tf.layers.conv2d(self.conv11,      256, (5, 5), (2, 2), padding = 'same', name = 'conv_layer_12'), alpha = 0.1) #(N_ROWS/64, N_COLS/64)

			self.conv13   = tf.nn.leaky_relu(tf.layers.conv2d(self.conv12,      512, (3, 3), (1, 1), padding = 'same', name = 'conv_layer_13'), alpha = 0.1) 
			self.conv14   = tf.nn.leaky_relu(tf.layers.conv2d(self.conv13,      512, (3, 3), (2, 2), padding = 'same', name = 'conv_layer_14'), alpha = 0.1) #(N_ROWS/128, N_COLS/128)
			self.conv15   = tf.nn.leaky_relu(tf.layers.conv2d(self.conv14,      512, (3, 3), (1, 1), padding = 'same', name = 'conv_layer_15'), alpha = 0.1)
			self.conv16   = tf.nn.leaky_relu(tf.layers.conv2d(self.conv15,      512, (3, 3), (1, 1), padding = 'same', name = 'conv-layer_16'), alpha = 0.1) #(N_ROWS/128, N_COLS/128)
